<template>
  <view class="product-container">
    ...
    <!-- 底部购物车条 -->
    <view class="cart-bar" @click="showCart.value = !showCart.value">
      <text>已选 {{ cartCount }} 件</text>
      <text>总计 ¥{{ cartTotal.toFixed(2) }}</text>
...
import { getProductList, addCartItem, getCart, updateCartItem, removeCartItem } from '../../api/shopping';
...
// 实时搜索
watch(() => searchParams.value.keyword, () => { loadProductList(0); });
...
const loadProductList = async (page = 0) => {
  try {
    ...
    currentPage.value = res.pageable?.pageNumber || page;
  } catch (e) {
    console.error(e);
...
// 分页
const changePage = page => { const p = Math.max(Math.min(page, totalPages.value - 1), 0); loadProductList(p); };
const toggleCart = () => {
  showCart.value = !showCart.value;
};

// 跳详情
const goToProductDetail = productId => { uni.navigateTo({ url: `/pages/product/detail?productId=${productId}` }); };

// 购物车
const cartItems = ref([]);
const showCart = ref(false);
const cartCount = computed(() => cartItems.value.reduce((s, i) => s + i.quantity, 0));
const cartTotal = computed(() => cartItems.value.reduce((s, i) => s + i.quantity * i.price, 0));

// 加入购物车
const addToCart = async (product) => {
  const exist = cartItems.value.find(i => i.productId === product.productId);
  if (exist) exist.quantity += 1;
  else cartItems.value.push({ productId: product.productId, name: product.name, price: product.price, quantity: 1 });
  try { await addCartItem(stageId.value, { productId: product.productId, quantity: 1 }); } catch (e) { console.error(e); }
};

// 增减购物车数量
const increment = async item => {
  item.quantity += 1;
  try { await updateCartItem(stageId.value, item.productId, item.quantity); } catch (e) { console.error(e); }
};
const decrement = async item => {
  if (item.quantity > 1) {
    ...
    try { await removeCartItem(stageId.value, item.productId); } catch (e) { console.error(e); }
  }
};
...
const loadCart = async () => {
  try {
    const res = await getCart(stageId.value); // res 已经是 data{}
    if (res.items) {
...
onLoad(async (options) => {
  if (!options.stageId) { uni.showToast({ title: '缺少 stageId 参数', icon: 'none' }); return; }
...
.cart-drawer{position:fixed;bottom:100rpx;left:0;right:0;max-height:400rpx;background:#fff;border-top:1rpx solid #ddd;box-shadow:0 -2rpx 12rpx rgba(0,0,0,0.1);padding:16rpx;overflow-y:auto;.cart-item{display:flex;justify-content:space-between;margin-bottom:12rpx} .checkout-btn{width:100%;padding:12rpx;background-color:#409eff;color:#fff;border-radius:12rpx;text-align:center}}
</style>
